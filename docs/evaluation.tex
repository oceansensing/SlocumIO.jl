...
\title{A Critical Evaluation of \texttt{dbdreader} \\
       \large and a Pure-Julia Replacement (\texttt{SlocumIO.jl})}
...
\texttt{SlocumIO.jl}, validated byte-for-byte against \texttt{dbdreader}'s
...
\texttt{SlocumIO.jl} adds a minutes-less-than-60 check.
...
\section{The \texttt{SlocumIO.jl} reimplementation}
...
\paragraph{Result.}
All 45 \texttt{(file, parameter)} combinations tested produced
\emph{identical} float64 arrays — same length, same min/max, same SHA-256
— covering DBD, SBD, MBD, EBD, and TBD file types for both gliders, and
parameters such as \texttt{m\_depth}, \texttt{m\_heading},
...
\texttt{sci\_water\_pressure}, and \texttt{sci\_m\_present\_time}.

\subsection{Bugs found during \texttt{SlocumIO.jl} development}
...
\begin{itemize}[leftmargin=*,topsep=1pt,itemsep=0pt]
  ...
  \item LZ4-compressed glider files (\texttt{.dcd}/\texttt{.ecd}/etc.) have
        unit tests against the LZ4 block format specification but have not
        been round-tripped through a real compressed glider file.
...
\texttt{SlocumIO.jl} reads the raw binary; it does not provide:
\begin{itemize}[leftmargin=*,topsep=1pt,itemsep=0pt]
