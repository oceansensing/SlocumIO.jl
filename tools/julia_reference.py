...
def _summarise(arr):
    """Summary dict for one float64 array, matching the reference JSON schema."""
    import numpy as np
    import hashlib
    ...
    return {
        ...
        "last3": [float(x) for x in a[-3:]],
        # SHA-256 over the raw little-endian float64 bytes, truncated to 16 hex
        # chars.  Byte-level, so it catches value differences that min/max miss.
        "sha256": hashlib.sha256(a.tobytes()).hexdigest()[:16],
    }


def fingerprint(filename, params, cachedir):
    """Return the `params` entry for one file: {param: {value: {...}, time: {...}}}."""
    out = get(filename, params, cachedir=cachedir)
    entry = {}
    for p in params:
        t, v = out[p]
        entry[p] = {"value": _summarise(v), "time": _summarise(t)}
    return entry
...
if __name__ == '__main__':
    ...
    ap = argparse.ArgumentParser(
        description="Emit reference fingerprints for SlocumIO.jl's test fixture. "
                    "This is an INDEPENDENT Python implementation of the same "
                    "algorithm; agreement with the Julia reader is a "
                    "cross-implementation check, not a dbdreader comparison.")
    ap.add_argument("datadir", help="directory holding the glider data files")
    ap.add_argument("-c", "--cachedir", default=None,
                    help="sensor-list cache dir (default: <datadir>/cache)")
    ap.add_argument("-o", "--output", default=None,
                    help="write JSON here (default: stdout)")
    args = ap.parse_args()

    cachedir = args.cachedir or os.path.join(args.datadir, "cache")

    # (filename, params) pairs covering the committed fixture.
    FIXTURE = [
        ("electa-2025-120-1-113.sbd",
         ["m_present_time", "m_depth", "m_gps_lat", "m_gps_lon",
          "m_heading", "m_pitch", "m_roll", "m_battery"]),
        ("electa-2025-120-1-141.tbd",
         ["sci_m_present_time", "sci_water_temp", "sci_water_cond",
          "sci_water_pressure"]),
    ]

    doc = {
        "_provenance": (
            "Generated by tools/julia_reference.py, an independent pure-Python "
            "implementation of the same algorithm. Agreement between it and the "
            "Julia reader is a cross-implementation regression check. It is NOT "
            "a comparison against dbdreader -- see test/reference_fingerprints.json "
            "for the historical dbdreader validation."
        ),
    }
    for fn, params in FIXTURE:
        path = os.path.join(args.datadir, fn)
        if not os.path.exists(path):
            raise SystemExit(f"fixture file missing: {path}")
        doc[fn] = {"filename": fn, "params": fingerprint(path, params, cachedir)}
    ...
    if args.output:
        ...
        for fn, params in FIXTURE:
            for p in params:
                s = doc[fn]["params"][p]["value"]
                print(f"  {fn:28s} {p:22s} n={s['n']:>6d}  sha={s['sha256']}")
    else:
        print(text)
